...
public class DbHelper{
    ...
    private int userId;
    private DbHelper(){}
    private static PersonInfo p=new PersonInfo();
    ...
    //    返回文章号
    public int insertPassage(int userid, String content, String imgPath, String posttime, String changetime , String postplace)
    {
        ContentValues cv = new ContentValues();
    ...
    public void insertUserInfo(Integer userid, String UserName, String UserInfo, String Sex, String head)//添加或更改用户信息
    {
        ...
        Log.d("p=:",Integer.toString((int)p));
        if(p!=-1)
        {
    ...
    public List<passage> searchPassage(int offset, int row, String username){
        final List<passage> passageinfo=new ArrayList<>();
        new Thread(new Runnable()//之后写成Activity里的thread
        {
            public void run()
            {
                try {
                    URL url=new URL("http://192.168.0.106:8080/getpassage");
    ...
    {
        Cursor cursor = db.rawQuery("select * from collect where CollectPassageId=?and CollectUserId=?",new String[]{Integer.toString(passageid),Integer.toString(userid)});
    ...
    public int getLikeNum(int passageid)
    {
        Cursor cursor = db.rawQuery("select * from passage where PassageId=?",new String[]{Integer.toString(passageid)});

        while (cursor.moveToNext()) {

           return  cursor.getInt(cursor.getColumnIndex("LikeNumber"));

        }cursor.close();
        return 0;
    }
    public int getCollectNum(int passageid)
    {
        Cursor cursor = db.rawQuery("select CollectNumber from passage where PassageId=?",new String[]{Integer.toString(passageid)});
        while (cursor.moveToNext()) {

            return  cursor.getInt(cursor.getColumnIndex("CollectNumber"));
    ...
    public PersonInfo getUserInfo(final int id){//获取用户名、头像、性别、介绍等
        //id = 190002;
        new Thread(new Runnable()//之后写成Activity里的thread
        {
            public void run()
            {
                try {
                    URL url=new URL("http://192.168.0.106:8080/getuser?i="+id);
                    HttpURLConnection connection=(HttpURLConnection)url.openConnection();
                    connection.setRequestMethod("POST");
                    connection.setDoOutput(true);
                    connection.setDoInput(true);
                    ...
                    if(responseCode == HttpURLConnection.HTTP_OK){
                        ...
                        p.set(user);//图片路径设置为：服务器IP/图片名
                        System.out.println("dbhelper:p"+p);
                        }
